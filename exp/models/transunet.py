...
class TransUNetWrapper(nn.Module):
    ...
    def forward(self, sar, optical, elevation, water_occur):
        o, loss1, loss2 = self.model(optical)  # optical only, unpack tuple
        return o  # return only the main output
...
class TransUNet(nn.Module):
    ...
    #    e1 = torch.cat([e1, F.interpolate(r1, size=e1.shape[2:], mode='bilinear', align_corners=False)], 1)
    #    e2 = torch.cat([e2, F.interpolate(r2, size=e2.shape[2:], mode='bilinear', align_corners=False)], 1)
    #    e3 = torch.cat([e3, F.interpolate(r3, size=e3.shape[2:], mode='bilinear', align_corners=False)], 1)
    #    e4 = torch.cat([e4, F.interpolate(r4, size=e4.shape[2:], mode='bilinear', align_corners=False)], 1)
       e1 = self.change1(e1)
       e2 = self.change2(e2)
       e3 = self.change3(e3)
       e4 = self.change4(e4)
